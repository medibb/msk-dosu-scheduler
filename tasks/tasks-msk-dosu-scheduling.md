...
- [ ] 2.0 환자/대기자 관리 (등록·수정·검색·필터, 상태 흐름, 엑셀 임포트)
  ...
  - [ ] 2.2 대기자 패널(`waitlist.html`): 상태/치료사/처방일자 필터·정렬, 등록번호·성명 검색
  - [ ] 2.3 환자 카드/상세: 상태 색상 구분, 마지막연락일·연락결과 기록
  - [ ] 2.4 상태 흐름(대기중→예약완료→시행중→종결) 1~2클릭 전환(액션 뷰)
  - [ ] 2.5 엑셀 임포트 파서(`services/import_excel.py`): 컬럼 매핑 + 테스트
  - [ ] 2.6 임포트 화면(`import.html`): 파일 업로드 → 미리보기 → 1회 일괄 등록

- [ ] 3.0 주간 시간표 그리드 (10타임 요일×시간, 드래그앤드롭 배정, 치료사별 보기)
  - [ ] 3.1 시간대 구성 로직(`services/schedule.py`): 기본 40분 10타임(09:00~11:40, 13:00~17:00), 설정 변경 가능 + 테스트
  - [ ] 3.2 그리드 렌더링(`schedule.html`): 요일(월~금)×10타임, 셀에 환자 카드
  - [ ] 3.3 SortableJS 드래그앤드롭(`schedule.js`) + 배정 AJAX 뷰 → 상태 자동 전환
  - [ ] 3.4 배정 환자 셀 간 이동(일정 변경)
  - [ ] 3.5 치료사별 보기 전환/전체 보기, "스케줄 고정"(매주 반복) 표시
  - [ ] 3.6 배정/이동 시 충돌(같은 치료사·같은 시간 중복) 방지 + 테스트

- [ ] 4.0 회차/진행 추적 (회차 증가, 종결 목표(기본 6회) 알림, 종결 처리)
  - [ ] 4.1 회차 로직(`services/sessions.py`): 회차 +1, 실시 날짜 기록(Session) + 테스트
  - [ ] 4.2 환자별 종결 목표 회차(기본 6회, 변경 가능) 필드/설정
  - [ ] 4.3 목표 회차 도달 시 종결평가 알림 표시, 종결평가 예정일 지정/표시
  - [ ] 4.4 상태 `종결` 전환 시 종결일 자동 기록
  - [ ] 4.5 환자 카드/상세에 회차 요약(예: 6회차/6) 노출

- [ ] 5.0 통계/리포트 (기간·처방코드·부위·치료사별 집계, CSV 내보내기)
  - [ ] 5.1 집계 로직(`services/stats.py`): 기간(월/분기/연) 총 건수, 처방코드별·부위별·치료사별 + 테스트
  - [ ] 5.2 통계 화면(`stats.html`): 기간 선택 + 집계 표
  - [ ] 5.3 CSV 내보내기
  - [ ] 5.4 최종 점검: 전체 흐름(임포트→대기→배정→회차→종결→통계) 수동 검증 + Docker 기동 확인
